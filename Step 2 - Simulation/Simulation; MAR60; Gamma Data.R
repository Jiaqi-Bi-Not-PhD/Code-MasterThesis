...
simulated_dataset_500_MAR_Data_Gamma20 <- readRDS("simulated_dataset_500_MAR60_Data_Gamma.RData")

Parameters <- c("log.lambda", "log.rho", "mgene", "newx_I", "log.kappa")
true_value <- c(-4.71, 0.804, 2.13, 1, 1.15) # log(1) - HighCorr; 1.15 - LowCorr
true_value2 <- c(exp(-4.71), exp(0.804), 2.13, 1, exp(1.15)) # exp(log(1)) - HighCorr; exp(1.15) - LowCorr
...
results_CCA |>
  ...
  summarise(MCSE_Bias = round(sd(Bias, na.rm = TRUE)/ sqrt(n_sim), 3),
            ...
            RMSE = round( sqrt(mean(Estimates - true_value, na.rm = TRUE)^2), 3), 
            Coverage = mean( (Estimates - 1.96 * SE <= true_value) & (true_value <= Estimates + 1.96 * SE ), na.rm = TRUE) )
...
sim_results_noKlogT <- process_datasets_K_H0T(
...
sim_results_noKlogT <- sim_results_noKlogT |>
  dplyr::select(SimulationID, everything())
sim_results_noKlogT <- sim_results_noKlogT |>
  dplyr::mutate(Method = methods)

n_sim <- length(unique(sim_results_noKlogT$SimulationID))

sim_results_noKlogT_toprint <- sim_results_noKlogT |> 
...
sim_results_noKlogT_toprint <- merge(sim_results_noKlogT_toprint, True_df, by = "Parameters")
sim_results_noKlogT_toprint <- sim_results_noKlogT_toprint |>
...
final_results <- list(n_sim, scenario, sim_results_noKlogT_toprint)
print(methods)
print(final_results)
...
## Start running the simulation
sim_results_noKH0T <- process_datasets_K_H0T(
  datasets = simulated_dataset_500_MAR_Data_Gamma20,  ## <- Change name accordingly here! ##
  true_value = true_value
)
...
## On some cases, dataset is abandoned due to model divergence
#nsim_left <- nrow(sim_results) / length(true_value)
sim_results_noKH0T <- sim_results_noKH0T |>
  dplyr::select(SimulationID, everything())
sim_results_noKH0T <- sim_results_noKH0T |>
...
sim_results_noKH0T_toprint <- sim_results_noKH0T |> 
  dplyr::group_by(Parameters) |> 
  dplyr::summarise(Ave.Est = round( mean(Estimates, na.rm = TRUE), 3 ),
                   MCSE_Bias = round(sd(Bias, na.rm = TRUE) / sqrt(n_sim), 3),
                   Bias = round( mean(Bias, na.rm = TRUE), 3 ),
...
sim_results_noKH0T_toprint <- merge(sim_results_noKH0T_toprint, True_df, by = "Parameters")
sim_results_noKH0T_toprint <- sim_results_noKH0T_toprint |>
  dplyr::select(c(Parameters, true_value, Ave.Est, Bias, MCSE_Bias, RMSE, MSE, EmpSE, Ave.ModSE, Coverage))

scenario <- "simulated_dataset_500_MAR_Data_Gamma20" ## Change name accordingly ##
final_results <- list(n_sim, scenario, sim_results_noKH0T_toprint)
print(methods)
print(final_results)
...
## Start running the simulation
sim_results_KlogT <- process_datasets_K_H0T(
  datasets = simulated_dataset_500_MAR_Data_Gamma20,  ## <- Change name accordingly here! ##
  true_value = true_value
)
...
## On some cases, dataset is abandoned due to model divergence
#nsim_left <- nrow(sim_results) / length(true_value)
sim_results_KlogT <- sim_results_KlogT |>
  dplyr::select(SimulationID, everything())
sim_results_KlogT <- sim_results_KlogT |>
  dplyr::mutate(Method = methods) ####### Change name here #######

n_sim <- length(unique(sim_results_KlogT$SimulationID))

sim_results_KlogT_toprint <- sim_results_KlogT |> 
  dplyr::group_by(Parameters) |> 
  dplyr::summarise(Ave.Est = round( mean(Estimates, na.rm = TRUE), 3 ),
                   MCSE_Bias = round(sd(Bias, na.rm = TRUE) / sqrt(n_sim), 3),
                   Bias = round( mean(Bias, na.rm = TRUE), 3 ),
...
sim_results_KlogT_toprint <- merge(sim_results_KlogT_toprint, True_df, by = "Parameters")
sim_results_KlogT_toprint <- sim_results_KlogT_toprint |>
  dplyr::select(c(Parameters, true_value, Ave.Est, Bias, MCSE_Bias, RMSE, MSE, EmpSE, Ave.ModSE, Coverage))

scenario <- "simulated_dataset_500_MAR_Data_Gamma20" ## Change name accordingly ##
final_results <- list(n_sim, scenario, sim_results_KlogT_toprint)
print(methods)
print(final_results)
...
## Start running the simulation
sim_results_KH0T <- process_datasets_K_H0T(
  datasets = simulated_dataset_500_MAR_Data_Gamma20,  ## <- Change name accordingly here! ##
  true_value = true_value
)
...
## On some cases, dataset is abandoned due to model divergence
#nsim_left <- nrow(sim_results) / length(true_value)
sim_results_KH0T <- sim_results_KH0T |>
  dplyr::select(SimulationID, everything())
sim_results_KH0T <- sim_results_KH0T |>
  dplyr::mutate(Method = methods) ####### Change name here #######

n_sim <- length(unique(sim_results_KH0T$SimulationID))

sim_results_KH0T_toprint <- sim_results_KH0T |> 
  dplyr::group_by(Parameters) |> 
  dplyr::summarise(Ave.Est = round( mean(Estimates, na.rm = TRUE), 3 ),
                   MCSE_Bias = round(sd(Bias, na.rm = TRUE) / sqrt(n_sim), 3),
                   Bias = round( mean(Bias, na.rm = TRUE), 3 ),
...
sim_results_KH0T_toprint <- merge(sim_results_KH0T_toprint, True_df, by = "Parameters")
sim_results_KH0T_toprint <- sim_results_KH0T_toprint |>
  dplyr::select(c(Parameters, true_value, Ave.Est, Bias, MCSE_Bias, RMSE, MSE, EmpSE, Ave.ModSE, Coverage))

scenario <- "simulated_dataset_500_MAR_Data_Gamma20" ## Change name accordingly ##
final_results <- list(n_sim, scenario, sim_results_KH0T_toprint)
print(methods)
print(final_results)
...
AllSimResults <- bind_rows(results_CCA, sim_results_noKlogT, 
                           ...
                           sim_results_KH0T)
saveRDS(AllSimResults, "MAR60 Gamma Well Specified; Simulation Results.RData")